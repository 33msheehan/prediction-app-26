  ...
  return (
    <ul className="space-y-3">
      {forecasts.map((forecast) => (
        <li
          className="rounded-xl border border-line bg-surface p-4 transition hover:border-line-strong"
          key={forecast.id}
        >
          <div className="flex items-start justify-between gap-4">
            <Link className="block flex-1 space-y-1" href={`/forecasts/${forecast.id}`}>
          ...
          {forecast.dueForReview ? (
            <Link
              className="mt-3 inline-block text-sm font-medium text-accent hover:opacity-80"
