...
| T6.1   | x       | x                     |                | Added `brier(p, outcome)` and `logScore(p, outcome)` in `lib/engine/scoring.ts`, with `logScore`'s `p` clamped to `[1e-10, 1-1e-10]` to avoid `±Infinity`. 8 unit tests cover the BUILD_PLAN §6.4-style known values (`brier(0.7,1)=0.09`, `brier(0.5,0)=0.25`, `logScore(0.7,1)≈0.3567`), perfect-call edge cases, symmetry, and clamping at the `p=0`/`p=1` extremes. Pure, no dependencies beyond `T0.1`. Lint/typecheck/build green. |

Tickets not listed above (T1.2 onward except T6.1, all of Phases 3–5, 7–8) are
not started — omit a row until work begins.
