...
| T4.1   | x       | x                     |                | Added `listForecastVersions(userId, forecastId)` in `lib/db/repository.ts`, scoped by ownership (throws `ForecastNotFoundError` for another user's forecast like the other repository functions), returning versions ordered by `versionNo` ascending with headline/SE/trials/source/rationale/timestamp. Exposed on `/forecasts/[id]` via a new `VersionHistory` component (newest-first list). 2 new repository tests (ordering+completeness, user-isolation) pass against the real Neon DB; component test covers ordering/empty state. Lint/typecheck/build green. |
| T4.3   | x       | x                     |                | Already implemented as part of T3.2's "due for review" badge: `lib/forecasts/cadence.ts` exports a pure `getNextDueAt(cadence, lastVersionAt)` / `isDueForReview()` for `none`/`interval`/`dates`, with `lib/forecasts/cadence.test.ts` covering interval rollover, multi-date rollover, and past/future comparisons. No new work needed beyond confirming coverage; reused as-is by T4.4's check-in entry point. |
| T4.4   | x       | x                     |                | Added a check-in flow at `/forecasts/[id]/check-in`: loads the forecast's current tree and renders `TreeEditorShell` in a new `mode="checkin"`, which labels the save action "Save check-in", posts `source:'checkin'` to the existing versions endpoint (extended to accept a client-supplied `source`, validated to `'checkin'` or falling back to `'edit'`), and redirects back to the forecast page on success. Navigating away without saving creates no version (save is the only write). Entry points added from the dashboard's "Due for review" badge (now a "Check in now" link) and a "Check in" link on the forecast page header. Updated/added route tests (checkin source pass-through, invalid-source fallback) and a `ForecastList` test for the new check-in link; lint/typecheck/full suite/build green. |

Tickets not listed above (T1.2 onward except those above, all of Phases 5–8)
are not started — omit a row until work begins.
...
| Phase 4 | not_ready     |          |             | T4.1, T4.3, T4.4 done (version history, cadence engine, check-in flow). T4.2 (chart), T4.5 (update-discipline guardrails), T4.6 (rationale display, depends on T4.5) remain — T4.2/T4.5 each carry a `Human:` note in BUILD_PLAN.md, so they're left for the user. Phase not ready for review until those land. |
| Phase 5 | not_ready     |          |             |                                                                                                                                                                                                                                                                                    |
| Phase 6 | not_ready     |          |             |                                                                                                                                                                                                                                                                                    |
| Phase 7 | not_ready     |          |             |                                                                                                                                                                                                                                                                                    |
| Phase 8 | not_ready     |          |             |                                                                                                                                                                                                                                                                                    |
...
On `claude/t4.1-t4.4-checkin-history` (PR open), Phase 4's T4.1 (version
history wiring), T4.3 (cadence engine — found already built as part of T3.2),
and T4.4 (check-in flow reusing the editor in a new `checkin` save mode) are
done. The remaining Phase 4 tickets (T4.2 chart, T4.5 guardrails, T4.6
rationale display) each need a human product/visual judgment call per
BUILD_PLAN.md and are left for the user.
...
3. Review and merge `claude/t4.1-t4.4-checkin-history` (T4.1/T4.3/T4.4).
4. T4.2, T4.5, and T4.6 need the user's product/visual judgment calls noted in
   BUILD_PLAN.md before they can be built.
